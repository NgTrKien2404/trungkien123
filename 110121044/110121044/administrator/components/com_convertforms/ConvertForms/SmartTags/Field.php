...
class Field extends SmartTag
{
	...
	public function fetchValue($key)
	{
		...
		// Support Smart Tags with field's ID, eg: {field.2}
		if (\is_numeric($fieldName))
		{
			// Convert {field.1} to {field.email}
			$fieldName = $this->getFieldNameByFieldID($fieldName);
		}
	...
	private function getFieldNameByFieldID($fieldID)
	{
		$foundField = current(array_filter($this->data['submission']->prepared_fields, function($field) use ($fieldID)
		{
			return (int) $field->options->get('key') == (int) $fieldID;
		}));

		if ($foundField)
		{
			return strtolower($foundField->options->get('name'));
		}
	}
}