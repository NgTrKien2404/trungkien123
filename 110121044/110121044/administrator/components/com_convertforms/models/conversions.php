...
class ConvertFormsModelConversions extends JModelList
{
    ...
    public function getListQuery()
    {
        ...
        if ($period = $this->getState('filter.period'))
        {
            ...
            if ($date_from)
            {
                $query->where($db->quoteName('a.created') . ' >= ' . $db->q(JHtml::date($date_from, 'Y-m-d 00:00:00', null)));
            }

            if ($date_to)
            {
                $query->where($db->quoteName('a.created') . ' <= ' . $db->q(JHtml::date($date_to, 'Y-m-d 23:59:59', null)));
            }
        }
