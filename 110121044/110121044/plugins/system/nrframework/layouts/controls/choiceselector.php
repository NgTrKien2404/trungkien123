...
JHtml::stylesheet('plg_system_nrframework/controls/choiceselector.css', ['relative' => true, 'version' => 'auto']);
?>
<div class="tf-choiceselector-control mode-<?php echo $mode; ?><?php echo $class; ?>">
	<?php
		...
		foreach ($choices as $key => $_value)
		{
			...
			?>
			<div class="tf-choiceselector-control--item<?php echo $pro ? ' pro' : ''; ?>"<?php echo $pro ? ' data-pro-only="' . JText::_($label) . '"' : ''; ?>>
					<?php echo $pro ? '<span class="pro">' . JText::_('NR_PRO') . '</span>' : ''; ?>
					...
					<label for="fpf-control-input-item_<?php echo $id; ?>">
						...
						<span class="text"><?php echo JText::_($label); ?></span>
					</label>
			</div>
			<?php
			$i++;
